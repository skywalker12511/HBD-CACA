...
// Mencegah mulai sebelum tombol diklik
...
const second = 1000,
  ...
  hour = minute * 60,
  day = hour * 24;
let countDown = new Date('Oct 22, 2025 01:05:00').getTime(),
...
if (distance < 0) {
  ...
  if (userInteracted) {
    ...
    btnPlay.addEventListener('click', _slideSatu, { once: true });
  }
}
...
const _slideSatu = function () {
  confetti(); // 🎉 confetti muncul bersamaan dengan slide pertama
...
// === Tombol "Gak" kabur ke posisi acak ===
const btnGak = document.getElementById("gak");

if (btnGak) {
  const moveButton = () => {
    const maxX = window.innerWidth - btnGak.offsetWidth - 20;
    const maxY = window.innerHeight - btnGak.offsetHeight - 20;
    const randomX = Math.floor(Math.random() * maxX);
    const randomY = Math.floor(Math.random() * maxY);
    btnGak.style.position = "fixed";
    btnGak.style.left = `${randomX}px`;
    btnGak.style.top = `${randomY}px`;
    btnGak.style.transition = "all 0.2s ease";
  };

  // Kabur kalau mouse diarahkan ke tombol
  btnGak.addEventListener("mouseenter", moveButton);

  // Kabur juga di layar sentuh (HP)
  btnGak.addEventListener("touchstart", (e) => {
